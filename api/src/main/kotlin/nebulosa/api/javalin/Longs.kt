...
inline fun Validator<Long>.min(min: Long) = check(LongMinCheck(min), "must be greater or equal to $min")
inline fun Validator<Long>.max(max: Long) = check(LongMaxCheck(max), "must be less or equal to $max")
inline fun Validator<Long>.range(range: LongRange) = check(LongRangeCheck(range), "must be between ${range.first} and ${range.last}")
inline fun Validator<Long>.range(min: Long, max: Long) = range(min..max)
inline fun Validator<Long>.positive() = check(LongPositiveCheck, "must be greater than 0")
inline fun Validator<Long>.positiveOrZero() = check(LongPositiveOrZeroCheck, "must be greater or equal to 0")
