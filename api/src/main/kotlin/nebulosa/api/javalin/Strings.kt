...
inline fun Validator<String>.minLength(min: Int) = check(CharSequenceMinLengthCheck(min), "length should be greater or equal to $min")
inline fun Validator<String>.maxLength(max: Int) = check(CharSequenceMaxLengthCheck(max), "length should be less or equal to $max")
...
internal data object CharSequenceNotEmptyCheck : Check<CharSequence?> {
    ...
    override fun invoke(p: CharSequence?) = p == null || p.isNotEmpty()
}
