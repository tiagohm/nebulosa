...
inline fun Validator<Int>.min(min: Int) = check(IntMinCheck(min), "must be greater or equal to $min")
inline fun Validator<Int>.max(max: Int) = check(IntMaxCheck(max), "must be less or equal to $max")
inline fun Validator<Int>.range(range: IntRange) = check(IntRangeCheck(range), "must be between ${range.first} and ${range.last}")
inline fun Validator<Int>.range(min: Int, max: Int) = range(min..max)
inline fun Validator<Int>.positive() = check(IntPositiveCheck, "must be greater than 0")
inline fun Validator<Int>.positiveOrZero() = check(IntPositiveOrZeroCheck, "must be greater or equal to 0")
