...
data class AutoFocus(
    ...
    fun add(path: Path) {
        if (exposureCount > 0) {
            exposureCount--
            LOG.d { debug("added. path={}, exposureCount={}", path, exposureCount) }
    ...
    fun determinate(focusPosition: Int): AutoFocusResult {
        ...
        LOG.d { debug("determinate. position={}, state={}, subState={}", focusPosition, currentState, subState) }
        ...
        when (currentState) {
            ...
            State.FocusPoints -> {
                ...
                if (remainingSteps > 0) {
                    ...
                    // Fim da captura.
                        ...
                        LOG.d { debug("HFD measured after exposures. hfd={}, stdDev={}", measurement.hfd, measurement.stdDev) }
            ...
            State.EvaluateTrendLinePoints -> {
                ...
                LOG.d { debug("trend line evaluated. left={}, right={}", leftCount, rightCount) }
            ...
            State.VerifyDataIsEnougth -> {
                ...
                if (maximumFocusPoints < focusPoints.size) {
                    ...
                    LOG.warn("failed to complete. Maximum number of focus points exceeded ({}).", maximumFocusPoints)
                    return AutoFocusResult.Failed(initialFocusPosition)
                }
                ...
                if (focusPosition <= 0 || focusPosition >= focusMaxPosition) {
                    ...
                    LOG.warn("failed to complete. position reached to min/max")
                    return AutoFocusResult.Failed(initialFocusPosition)
                }
    ...
    private fun moveFocuser(position: Int, relative: Boolean): AutoFocusResult {
        ...
        LOG.d { debug("moving focuser. position={}, relative={}", position, relative) }
    ...
    private fun takeExposure(): AutoFocusResult {
        ...
        LOG.d { debug("taking exposure. exposureCount={}", exposureCount) }
    ...
    private fun measureStars(path: Path): MeasuredStars {
        ...
        LOG.d { debug("detected {} stars", detectedStars.size) }
        val measurement = detectedStars.measureDetectedStars()
        LOG.d { debug("HFD measured. hfd={}, stdDev={}", measurement.hfd, measurement.stdDev) }
    ...
    private fun computeCurvePoint(): CurvePoint? {
        return if (measurement.hfd == 0.0) {
            ...
            LOG.d { debug("focus point added. remainingSteps={}, point={}", remainingSteps, focusPoint) }
    ...
    private fun validateCalculatedFocusPosition(focusPoint: CurvePoint): Boolean {
        LOG.d { debug("validating calculated focus position. threshold={}, x={}, y={}", rSquaredThreshold, focusPoint.x, focusPoint.y) }
        ...
        if (rSquaredThreshold > 0.0) {
            ...
            if (isBad) {
                LOG.warn("coefficient of determination is below threshold")
                return false
            }
        }
        ...
        if (focusPoint.x < min || focusPoint.x > max) {
            LOG.warn("determined focus point position is outside of the overall measurement points of the curve")
            return false
        }
