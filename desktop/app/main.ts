...
const apiHost = serve ? 'localhost' : parsed.values.host || 'localhost'
...
function createWindow(options: OpenWindow<any>, parent?: BrowserWindow) {
	...
	window.on('close', () => {
		...
		if (window === homeWindow?.window || id === homeWindow?.options?.id) {
			...
			for (const [, value] of browserWindows) {
				value.window.close()
			}
...
function findWindowById(id: number | string) {
	for (const [, window] of browserWindows) if (window.window.id === id || window.options.id === id) return window
	for (const [, window] of modalWindows) if (window.window.id === id || window.options.id === id) return window
	return undefined
}
