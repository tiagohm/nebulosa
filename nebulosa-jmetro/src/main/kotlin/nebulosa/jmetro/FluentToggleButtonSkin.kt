...
class FluentToggleButtonSkin(button: ToggleButton) : ToggleButtonSkin(button) {
    ...
    companion object {
        ...
        @JvmStatic private val SHRINK_ANIMATE_ON_PRESS_META_DATA = object : CssMetaData<ToggleButton, Boolean>(
            ButtonAnimationHelper.SHRINK_ANIMATE_ON_PRESS_PROPERTY_NAME,
            BooleanConverter.getInstance(), true
        ) {
            override fun isSettable(button: ToggleButton): Boolean {
                val skin = button.skin as FluentToggleButtonSkin
                return !skin.shrinkAnimateOnPressProperty.isBound
            }

            override fun getStyleableProperty(button: ToggleButton): StyleableProperty<Boolean> {
                val skin = button.skin as FluentToggleButtonSkin
