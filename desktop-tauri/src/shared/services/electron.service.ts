...
export class ElectronService {
	...
	async openDirectory(data?: OpenDirectory): Promise<string | false> {
	...
	async saveJson<T>(data: SaveJson<T>): Promise<JsonFile<T> | false> {
		...
		data.path = data.path || (await this.saveFile({ ...data, windowId: data.windowId ?? window.id })) || undefined
	...
	async openJson<T>(data?: OpenFile): Promise<JsonFile<T> | false> {
		const filters = [{ name: 'JSON files', extensions: ['json'] }]
		const path = await this.openFile({ ...data, windowId: data?.windowId ?? window.id })
	...
	async writeJson<T>(json: JsonFile<T>): Promise<boolean> {
		// return this.send('JSON.WRITE', json)
		return false
	}

	async readJson<T>(path: string): Promise<JsonFile<T> | false> {
	...
	async fullscreenWindow(enabled?: boolean): Promise<boolean> {
		// return this.send('WINDOW.FULLSCREEN', { enabled, windowId: window.id })
		return false
	}

	async closeWindow<T = unknown>(data?: unknown, id?: string): Promise<T> {
