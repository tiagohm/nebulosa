...
export class CameraComponent implements AfterContentInit, OnDestroy, Tickable {
	...
	cameraModel: SlideMenuItem[] = [this.ditherMenuItem, this.liveStackingMenuItem, this.namingFormatMenuItem, this.snoopDevicesMenuItem]

	running = false
	hasDewHeater = false
	preference = structuredClone(DEFAULT_CAMERA_PREFERENCE)
	request = this.preference.request
	mode: CameraMode = 'CAPTURE'

	dither: CameraDitherDialog = {
		showDialog: false,
		request: this.request.dither,
	}

	liveStacking: CameraLiveStackingDialog = {
		showDialog: false,
		request: this.request.liveStacking,
	}

	namingFormat: CameraNamingFormatDialog = {
		showDialog: false,
