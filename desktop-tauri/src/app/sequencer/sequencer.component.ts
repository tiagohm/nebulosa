...
export class SequencerComponent implements AfterContentInit, OnDestroy, Tickable {
	...
	property = structuredClone(DEFAULT_SEQUENCE_PROPERTY_DIALOG)
	preference = structuredClone(DEFAULT_SEQUENCER_PREFERENCE)
	calibrationGroups: DropdownItem<string | undefined>[] = []
	plan = this.preference.plan
	event?: SequencerEvent
	running = false
	path?: string

	// NOTE: Remove the "plan.sequences.length <= 1" on layout if add more options
	sequenceModel: SlideMenuItem[] = [
		{
	...
	drop(event: CdkDragDrop<Sequence[]>) {
	...
	async showRotatorDialog(sequence: Sequence) {
	...
	showSequenceMenu(sequence: Sequence, dialogMenu: DialogMenuComponent) {
	...
	moveSequenceTo(direction: 'TOP' | 'BOTTOM') {
	...
	deleteSequence(sequence: Sequence, index: number) {
	...
	duplicateSequence(sequence: Sequence, index: number) {
		this.plan.sequences.splice(index + 1, 0, structuredClone(sequence))
		this.savePreference()
	}

	filterRemoved(sequence: Sequence) {
		sequence.filterPosition = 0
		this.savePreference()
	}

	angleRemoved(sequence: Sequence) {
		sequence.angle = -1
		this.savePreference()
	}
