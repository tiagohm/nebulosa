...
export class ImageComponent implements AfterViewInit, OnDestroy {
	preference = structuredClone(DEFAULT_IMAGE_PREFERENCE)
	solver = structuredClone(DEFAULT_IMAGE_SOLVER_DIALOG)
	starDetector = structuredClone(DEFAULT_STAR_DETECTOR_DIALOG)
	transformation = this.preference.transformation
	fov = structuredClone(DEFAULT_IMAGE_FOV_DIALOG)
	annotation = structuredClone(DEFAULT_IMAGE_ANNOTATION_DIALOG)
	imageROI = structuredClone(DEFAULT_IMAGE_ROI)
	saveAs = structuredClone(DEFAULT_IMAGE_SAVE_DIALOG)
	statistics = structuredClone(DEFAULT_IMAGE_STATISTICS_DIALOG)
	mouseCoordinate = structuredClone(DEFAULT_IMAGE_MOUSE_COORDINATES)
	liveStacking = structuredClone(DEFAULT_IMAGE_LIVE_STACKING)
	zoom = structuredClone(DEFAULT_IMAGE_ZOOM)
	settings = structuredClone(DEFAULT_IMAGE_SETTINGS_DIALOG)
	...
	stretch: ImageStretchDialog = {
		showDialog: false,
		transformation: this.transformation.stretch,
	}

	scnr: ImageSCNRDialog = {
		showDialog: false,
		transformation: this.transformation.scnr,
	}

	astronomicalObject: AstronomicalObjectDialog = {
		showDialog: false,
	}

	headers: ImageHeadersDialog = {
		showDialog: false,
		headers: [],
	}
	...
	drawDetectedStar(star: DetectedStar) {
	...
	annotationSelected(selected?: ImageAnnotation) {
	...
	applyAutoStretchMeanBackground() {
		if (!this.stretch.transformation.auto) {
			return this.toggleStretch()
		} else {
			return this.loadImage()
	...
	restoreAutoStretchMeanBackground() {
		this.stretch.transformation.meanBackground = 0.5
