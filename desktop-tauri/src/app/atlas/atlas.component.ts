...
export class AtlasComponent implements OnInit, AfterContentInit, AfterViewInit, OnDestroy {
	...
	moon = structuredClone(DEFAULT_MOON)
	planet = structuredClone(DEFAULT_PLANET)
	minorPlanet = structuredClone(DEFAULT_MINOR_PLANET)
	skyObject = structuredClone(DEFAULT_SKY_OBJECT)
	satellite = structuredClone(DEFAULT_SATELLITE)
	preference = structuredClone(DEFAULT_SKY_ATLAS_PREFERENCE)
	refresh = structuredClone(DEFAULT_BODY_TAB_REFRESH)
	dateTimeAndLocation = structuredClone(DEFAULT_DATE_TIME_AND_LOCATION)

	tab = BodyTabType.SUN
	locations: Location[] = [structuredClone(DEFAULT_LOCATION)]
	...
	async minorPlanetSelected(event: ListboxChangeEvent) {
	...
	async closeApproachesOfMinorPlanets() {
		this.refresh.position = true
	...
	deleteFavorite(favorited: FavoritedSkyBody) {
	...
	async selectFavorite(favorited: FavoritedSkyBody) {
	...
	async dateTimeChanged(dateChanged: boolean, date?: Date) {
		if (date) {
