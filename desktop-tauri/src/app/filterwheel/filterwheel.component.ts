...
export class FilterWheelComponent implements AfterContentInit, OnDestroy, Tickable {
	wheel = structuredClone(DEFAULT_WHEEL)
	request = structuredClone(DEFAULT_CAMERA_START_CAPTURE)
	preference = structuredClone(DEFAULT_WHEEL_PREFERENCE)
	...
	async wheelChanged(wheel?: Wheel) {
		if (wheel?.id) {
	...
	focusOffsetForFilter(filter: Filter) {
