...
export class CalibrationComponent implements AfterViewInit, OnDestroy {
	frames = new Map<string, CalibrationFrame[]>()
	preference = structuredClone(DEFAULT_CALIBRATION_PREFERENCE)
	groupDialog = structuredClone(DEFAULT_CALIBRATION_GROUP_DIALOG)
	selectedFrames: CalibrationFrame[] = []
	...
	async openFilesToUpload(group: string) {
	...
	async openDirectoryToUpload(group: string) {
	...
	showEditGroupDialog(value: CalibrationFrame | string) {
