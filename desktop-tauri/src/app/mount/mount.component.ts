...
export class MountComponent implements AfterContentInit, OnDestroy, Tickable {
	...
	remoteControl = structuredClone(DEFAULT_MOUNT_REMOTE_CONTROL_DIALOG)
	preference = structuredClone(DEFAULT_MOUNT_PREFERENCE)
	currentComputedLocation = structuredClone(DEFAULT_COMPUTED_LOCATION)
	targetComputedLocation = structuredClone(DEFAULT_COMPUTED_LOCATION)
	...
	slewRate?: SlewRate
	slewingDirection?: MountSlewDirection
	...
	targetCoordinateModel: SlideMenuItem[] = [
		{
	...
	async stopRemoteControl(protocol: MountRemoteControlProtocol) {
	...
	moveTo(direction: MountSlewDirection, pressed: boolean, event?: MouseEvent) {
