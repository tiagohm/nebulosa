...
export class HomeComponent implements AfterContentInit {
	preference = structuredClone(DEFAULT_HOME_PREFERENCE)
	connection?: ConnectionDetails
	connectionDialog = structuredClone(DEFAULT_HOME_CONNECTION_DIALOG)
	...
	deviceMenuToolbarBuilder = (device: Device): MenuItem[] => {
		if (isCamera(device)) {
			return [
				{
					icon: 'mdi mdi-image',
	...
	deviceConnected(event: DeviceConnectionCommandEvent) {
		return DeviceChooserComponent.handleConnectDevice(this.api, event.device, event.item)
	}

	deviceDisconnected(event: DeviceConnectionCommandEvent) {
