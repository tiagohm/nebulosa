...
object FitsHeaderCardFormatter {
    ...
    private fun StringBuilder.appendKey(card: HeaderCard) {
        ...
        padTo(FitsHeaderCard.MAX_KEYWORD_LENGTH)
    }
    ...
    private fun StringBuilder.realign(at: Int, from: Int): Boolean {
        return if (length >= FitsHeaderCard.FITS_HEADER_CARD_SIZE || from >= FitsHeader.commentAlignPosition) {
            // We are beyond the alignment point already...
            false
        } else {
            realign(at, from, FitsHeader.commentAlignPosition)
        }
    }
    ...
    private fun StringBuilder.padTo(to: Int) {
        for (pos in length % FitsHeaderCard.FITS_HEADER_CARD_SIZE until to) {
            append(' ')
        }
    }
    ...
    private fun StringBuilder.availableCharCount(): Int {
        return (FitsHeaderCard.FITS_HEADER_CARD_SIZE - length % FitsHeaderCard.FITS_HEADER_CARD_SIZE) % FitsHeaderCard.FITS_HEADER_CARD_SIZE
    }
